package com.shestnut.user.internal.common.utils.json;
...
public class JsonUtil {
    ...
    public static <T> T json2objectDepth(String json, Class<T> clazz, Class<?>... genericClazz) {
        ...
        if ("".equals(json)) {
            return (String.class == clazz) ? (T) "" : null;
        }
        try {
    ...
    private static String ignoreChars(String chars) {
        if (chars == null) {
            return "";
        } else {
            int ignoredChars = chars.length() - 128;
            if (ignoredChars > 0) {
                String output = chars.substring(0, 128);
